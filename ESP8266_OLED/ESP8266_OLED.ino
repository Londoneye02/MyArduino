...
/*
 SCL// SCLK:D5
 SDA//MOSI:D7
 RESET A VCC CON RESISTENCIA 10K
*/
...
//
//  Serial.print("Lines                    ");
//  Serial.println(testLines(CYAN,RED));
//  delay(1000);
////
...
unsigned long testLines(uint16_t color,uint16_t color2) {
  ...
  x2    = w - 1;
  for (y2 = 0; y2 < h; y2 += 6) tft.drawLine(x1, y1, x2, y2, color2);
...
unsigned long testFastLines(uint16_t color1, uint16_t color2) {
  ...
  tft.clearScreen();
  start = micros();
  for (y = 0; y < h; y += 5) tft.drawFastHLine(w/4, y, w/2, color1);//RED
  for (x = 0; x < w; x += 5) tft.drawFastVLine(x, h/4, h/2, color2);//BLUE
...
unsigned long testRects(uint16_t color) {
  ...
  for (i = n; i >n-8; i -= 6) {
    i2 = i / 2;
    tft.drawRect(cx - i2-20, cy - i2, i, i, color);
    delay (100);
  }
